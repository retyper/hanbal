...
const VIEW = {
  ...
  /**
   * 최소 배율 (px/m). 이보다 더 줌아웃하지 않는다.
   *
   * 8 → 3.2 (2026-08-31, 형: "5-9에서는 너무 멀어서 화면 벗어나니까 그렇게 되지 않도록 해").
   * 5장은 55~115m 다. 115m 스팬을 폰(가로 355px)에 담으려면 3.1px/m 가 필요한데 하한이 8이라
   * **과녁이 화면 밖에 그려지고 있었다** — 줌아웃을 막는 하한이 오히려 화면을 깨고 있었다.
   * 하한을 내려도 과녁이 점이 되지 않는 이유: 반경이 거리에 비례하기 때문이다
   * (stagekit.angularSize · enemy.foeR). 112m 적의 반경은 0.62m 가 아니라 1.5m 다.
   */
  minScale: 3.2,
  maxScale: 110,
