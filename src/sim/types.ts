...
export interface ArcherState {
  ...
  aimAngle: number
  /**
   * 겨눈 자리까지의 거리 (m). 각만으로는 '어디'를 모른다 — 집중의 탄도해가 이걸 쓴다.
   * 커서가 궁수 뒤로 가서 각이 클램프되면, 클램프된 방향의 같은 거리를 겨눈 것으로 본다.
   */
  aimDist: number
  ...
  /**
   * 집중 0..1 — **만작 뒤에 차오르는 두 번째 게이지** (2026-08-31).
   *
   * 왜 생겼나: 만작에 닿으면 힘(draw)은 상한이고, 그 뒤로 참는 것은 **100% 손해**였다.
   * 그래서 최적 플레이가 언제나 "만작 뜨자마자 놓기"였고, 이 게임의 유일한 독창 메커닉인
   * 떨림·빨간 바·호흡정지 구역에 **아무도 자발적으로 안 들어갔다.** 선택한 적 없는 메커닉은
   * 메커닉이 아니라 벽이다.
   *
   * 무엇을 하는가: 차오르는 만큼 **발사각이 조준선에서 탄도해로 옮겨간다.**
   * 0이면 겨눈 방향 그대로 나가 중력에 떨어지고(지금까지의 게임), 1이면 겨눈 **자리**에 꽂힌다.
   * 즉 먼 거리에서 이 게이지가 곧 명중률이다.
   *
   * 대가: 차는 동안 스태미나가 계속 줄어 빨간 바 아래로 내려가고(=떨린다), 다 차기 전에
   * 바닥나면 붕괴다. 호흡정지로 떨림을 죽일 수 있지만 그동안 스태미나는 더 빨리 준다.
   * 스탯 '집중'(progression.ts focus)이 숨을 참는 한계를 늘린다 — **이름이 같은 게 맞다.**
   */
  focus: number
...
export type TargetKind =
   ...
   * 맞다. 이건 쏘아 맞히는 **점수판**이 아니라 판에 놓인 **물건**이다. 그래서 셋이 다르다:
   *   1. 클리어 조건에 안 들어간다 (world.ts anyTargetStanding) — 안 터뜨려도 판은 끝난다.
   *   2. 점수를 안 준다 — 값은 폭발이 끊는 것들이 낸다.
   *   3. 좌상단 '남은 것' 숫자에도 안 센다 (render/hud.ts).
   * 맞으면 무조건 죽고 터진다 (bomb 과 같은 함수, target.ts burst).
   */
  | 'barrel'
  /** 좌우/상하 이동 */
  | 'moving'
...
export interface Target {
   ...
   * 갑옷을 아직 입고 있는가.
   *
   * ★ 예전엔 이게 **영구 상태**였다 — 갑옷 적의 몸통은 관통살이 아니면 영원히 안 통했다.
   *   형의 반려 (2026-08-31): *"적 갑옷병은 갑옷도 무적이 아니게."* 옳다. 무적인 부위는
   *   선택지를 지운다: 관통살이 없으면 답이 헤드샷 하나로 줄고, 그건 자물쇠가 아니라 벽이다.
   *   이제 갑옷은 **깨진다** — armorHp 를 다 깎으면 이 값이 false 가 되고, 그 뒤로는
   *   아무 살이나 통한다. 갑옷은 이제 '못 뚫는 것'이 아니라 '먼저 벗겨야 하는 것'이다.
   */
  armored: boolean
  /**
   * 남은 갑옷 내구도. 0 이하가 되는 순간 armored 가 false 로 떨어진다.
   * 보통 살의 몸통샷이 여기를 깎는다 — 체력에는 안 닿지만 **헛발이 아니다.**
   */
  armorHp: number
  /** 판 시작 시점의 갑옷 내구도. 화면이 그릴 게이지의 분모다. */
  armorMax: number
...
export type SimEvent =
  ...
  | { t: 'full_draw' }
  /**
   * 집중이 가득 찼다 — 이 한 발은 겨눈 자리로 간다.
   * 화면·소리가 이 순간을 표시한다. 판정은 이 이벤트가 아니라 archer.focus 가 한다.
   */
  | { t: 'full_focus' }
   ...
   * 화면은 이 값으로 "막혔다"와 "깎이고 있다"를 다르게 그린다 — 진행이 안 보이면
   * 플레이어는 그 벽을 계속 때릴 이유를 못 찾는다.
   */
  ...
  | { t: 'molgi'; on: boolean }
  /**
   * 적이 쓰러졌다 — **연출 전용 사건.** sim은 이걸 다시 읽지 않는다.
   *
   * 형: "과녁은 없어지더라도 적군들은 죽었을때 없어져버리지 말고 애니메이션과 함께
   *      시체로 남거나 드론은 망가져 떨어지거나 해야해. 화살맞은 에너지와 물체의 질량에
   *      따라 퍽하고 뜨거나 날아가거나 물리엔진 적용해서 말이야."
   *
   * 그래서 여기 실리는 건 '시체'가 아니라 **충격**이다: 이 죽음을 만든 것의 속도(vx·vy, m/s)와
   * 질량비(mass). 몸이 얼마나 뜨고 어디로 나뒹구는지는 **렌더가** 정한다
   * (render/effects.ts) — 시체는 판정에 아무 영향이 없으므로 sim에 상태를 만들지 않는다 (A1).
   * look: 0 들판 사수 · 1 창문의 사수 · 2 숨는 사수 · 3 드론 · -1 보스.
   */
  | {
