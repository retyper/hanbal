...
export interface ArcherState {
  ...
  aimAngle: number
  /**
   * 겨눈 자리까지의 거리 (m). 각만으로는 '어디'를 모른다 — 화면·디버그가 읽는다.
  ...
  parryHeld: boolean
  /**
   * 이번 휘두름이 **올려베기**인가 (2026-09-10, 형: "다음 타격은 내려치기가 되고 다음은 다시
   * 올려치기가 되고"). 한 번 휘두를 때마다 뒤집힌다.
   *
   * 왜 sim 에 있나: 렌더가 세면 판을 다시 시작하거나 탭을 다녀올 때 위아래가 어긋난다.
   * 같은 시드가 같은 그림을 내야 한다 (A1) — 자세도 그림의 일부다.
   */
  parryUp: boolean
}
...
export type SimEvent =
  ...
  /**
   * 눌렀는데 **아직 쉬는 중이라** 칼이 안 나갔다 (2026-09-10 feel-lens ④).
   * 예전엔 이 순간에 소리도 자세도 버튼 색도 없어서 "게임이 씹었다"로 읽혔다.
   * 낮고 짧은 소리 하나면 된다 — 벌이 아니라 **아직**이라는 말이다.
   */
  | { t: 'parry_fail' }
  /**
   ...
   * look: 0 들판 사수 · 1 창문의 사수 · 2 숨는 사수 · 3 매 · 4 화차 · 5 총통수 · 6 투석군 · **음수 = 보스** (−1 − 보스의 look).
   */
  | {
...
export interface EnemyShot {
   ...
   *   0 화살 (사람이 쏜 것) · 1 **돌** (매가 놓은 것 · 투석군이 던진 것) ·
   *   2 **신기전** (화차) · 3 **혼불** (보스 귀신) · 4 **탄환** (승자총통)
   * 판정은 다섯이 같다 — 다른 건 그림과 소리뿐이다. 규칙이 하나여야 배우는 것도 하나다.
   */
  look: number
}
