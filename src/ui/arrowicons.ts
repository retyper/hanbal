...
export const ARROW_ICON: Record<ArrowKindId, string> = {
  /**
   * 유엽전(柳葉箭) — **화살이지 화살표가 아니다** (2026-09-11, 형: "진짜 무슨 화살표처럼
   * 만들어버리니까 화살표랑 헷갈리잖아. 똑바로 다시 그려").
   *
   * 옛 그림은 〈가로줄 + 꺾쇠〉였다. 그건 "다음으로" 를 뜻하는 UI 글리프지 물건이 아니다.
   * 화살을 화살로 만드는 것은 **양 끝**이다:
   *   촉  버들잎(柳葉) — 이름 그대로 잎사귀 모양. 꺾쇠가 아니라 **닫힌 잎**이다.
   *   오늬 쪽에 **깃 둘과 오늬 홈** — 이 셋이 붙는 순간 방향 표시와 안 헷갈린다.
   */
  basic:
    '<path d="M8.5 14h10.5"/>' +
    '<path d="M19 14q3.2-3 6.4 0q-3.2 3-6.4 0z"/>' +
    '<path d="M8.5 14 4.6 10.4M8.5 14 4.6 17.6"/>' +
    '<path d="M3.4 11.6v4.8"/>',
