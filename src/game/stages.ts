 ...
 *
 * 형: "보스깨면 죽었을때 직전보스 다음스테이지부터 시작하게."
 *
 * ★ 2026-09-10 정정 (형: "죽은 다음에 게임 이어서 하니까 **해본 적도 없는 20-1**로 넘어가네").
 *   예전엔 이 값을 `save.bossKills`(누적 처치 수)로 셌고, 주석에 "마디는 순서대로만 열리므로
 *   누적 수가 곧 깊이"라고 적어 두었다. **그 전제가 틀렸다** — 10판에서 죽고 체크포인트에서
 *   다시 시작해 10판 보스를 **또** 잡으면 누적 수가 2가 되어 20판이 열린다. 한 번도 가본 적
 *   없는 판이다. 그래서 이제 `save.bossDepth`(잡아본 것 중 **가장 깊은 마디**, 줄지 않는다)를
 *   받는다. 깊이는 깊이로 세야 한다.
 *
 * depth=0 → 0(=1-1) · 1 → 10(=2-1) · 2 → 20(=3-1) · … 캠페인을 넘어서도 그대로
 * 이어진다(끝없는 구간도 10판마다 보스라 같은 식이 통한다).
 */
export function checkpointStage(bossDepth: number): number {
  const n = Math.floor(bossDepth)
