 ...
 */
// ── 활이 열리는 순서 (2026-08-31, 형: "밸런스에 맞는 활들을 순서대로 개방되게 해라") ──
//
// 예전 순서는 각궁(10) → 장궁(30) → 리커브(60) → 컴파운드(100) 였다. 그런데 각궁이
// 밸런스 시뮬에서 62판 중 38판 1등인 지배 활이었다 — **가장 센 활이 가장 먼저** 열렸고,
// 그 뒤에 열리는 활들은 전부 하향된 물건처럼 보였다. 해금이 상이 아니라 형벌이었다.
//
// 이제 순서는 **그 활이 요구하는 것**을 따라간다:
//   리커브(안정) → 컴파운드(오래 잡기) → 장궁(사거리) → 각궁(공격성·예민함)
// 각 활이 그 시점에 막 배운 것과 맞물린다: 컴파운드는 집중을 배운 뒤, 장궁은 곡사(5장)를
// 겪은 뒤, 각궁은 빨간 바 아래를 다룰 줄 알게 된 뒤에 온다.
const G_RECURVE_STAGE = 10
const G_COMPOUND_STAGE = 30
const G_LONGBOW_STAGE = 60
const G_GAKGUNG_STAGE = 100
