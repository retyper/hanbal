...
/**
 * ── 화폐에는 **이름이 없다** (2026-09-11, 형: "냥이란 단어 쓰지 말고 엽전과 숫자만 해") ──
 *
 * 처음엔 '냥(兩)' 이라는 말을 붙였다. 시대에 맞는 말이긴 한데, 화면에서는 **읽을 것이 하나 늘 뿐**이다.
 * 엽전 그림이 이미 "이건 돈이다"를 말하고 있으니 그 옆의 글자는 군더더기다.
 * 게임의 돈은 대개 이렇게 생겼다 — 아이콘 하나, 숫자 하나.
 *
 * 그래서 이 파일에는 이제 이름이 없다. **그림(drawCoin·coinHtml)과 숫자(coinText)뿐이다.**
 */
...
/**
 * 값 하나를 글자로 — **숫자뿐이다.** 단위를 안 붙인다 (위 주석).
 * 그림이 들어갈 수 있는 자리에는 `coinHtml()` 을 써서 엽전을 같이 보여준다.
 */
