 ...
 *
 * ── 2026-09-10, 형의 결정 ────────────────────────────────────────────
 * **"번들 억지로 크기 왜 자꾸 줄이냐고. 효과적인 소리 이미지 다 담으려면 크기는 2순위야."**
 *
 * 그래서 110/300 → **150/400** 으로 올린다. 이 상한의 뜻이 바뀐 것은 아니다 — 여전히
 * "라이브러리를 몰래 들였는지" 알아채는 경보기다(A6). 다만 **소리와 그림 때문에 기능을 깎는
 * 일은 없다.** 실제로 소리·그림·글꼴은 public/ 에 있어 이 숫자에 안 들어간다 (번들 밖) —
 * 여기서 재는 건 JS 뿐이다. 그러니 이 상한이 진짜로 막아야 할 것은 코드의 군살 하나다.
 *
 * ── 2026-09-20, 형이 **두 번째로** 말했다 ─────────────────────────────
 * **"아니 왜이렇게 번들 크기에 연연하는거야. 가벼울수록 좋은게 맞지만 그렇게 까지 강박적으로 가벼워야할 이유가 없어.
 *   항상 게임퀄리티가 우선이야."**
 *
 * 여유가 8KB 남았다고 "다음엔 뭘 걷어내야 한다"고 보고했다가 들은 말이다. 맞다 — 391KB 의 JS 는 어떤 기기에서도 문제가 아니고,
 * 이 숫자 때문에 화면을 덜 만들 이유는 없다. 150/400 → **300/800**. 이 상한은 이제 **경보기일 뿐**이다 (라이브러리를 몰래 들였거나
 * 같은 코드가 두 번 묶였을 때 울린다). 여유가 줄었다는 것을 작업 보고의 걱정거리로 올리지 말 것.
 */
const LIMIT = {
